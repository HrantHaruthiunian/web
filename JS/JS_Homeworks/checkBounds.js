...
function intWithinBounds() {
   ...
   return function () {
      ...
      if (givenNumber > lowerNumber && givenNumber < upperNumber) {
         ...
         let result = `<h3>You have inputed the following interval: (${lowerNumber}, ${upperNumber}). 
         As well as a number:   ${givenNumber}.<br> 
         So, the given number is WITHIN the interval. </h3><hr> ${fileRefPresentation}`;
         return result;

      } else if (givenNumber <= lowerNumber || givenNumber >= upperNumber) {
         let result = `<h3>You have inputed the following interval: (${lowerNumber}, ${upperNumber}). 
         As well as a number:   ${givenNumber}.<br> 
         So, the given number is NOT WITHIN the interval. </h3><hr> ${fileRefPresentation}`;
         return result;
